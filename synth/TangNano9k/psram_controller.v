...
// Feng Zhou, 2022.8
//
// Vendored verbatim from https://github.com/zf3/psram-tang-nano-9k
// (Apache License 2.0, see LICENSE-psram_controller.txt in this directory).
//
...
module PsramController #(
    ...
    // DEBUG: read back the CR0 register we configure at boot (CONFIG_ST),
    // to check whether that write actually took effect on the real chip -
    // see top_tangnano9k.vhd for why this matters (a silent CR/LATENCY
    // mismatch corrupts memory writes with no error flag).
    input      cr_read,        // pulse (from IDLE_ST only) to start a CR0 readback
    output reg [7:0] cr_read_data,
    output reg cr_read_valid,  // pulses once when cr_read_data is fresh
...
localparam [2:0] CR_READ_ST = 3'd5;
...
always @(posedge clk) begin
    ...
    if (state == IDLE_ST) begin
        ...
        end else if (cr_read) begin
            // Same CA as CONFIG_ST's CR0 write (8'h60,...), but with the
            // R/W# bit (CA bit 47, top bit of the first CA byte) flipped to
            // 1 = read: 8'h60 -> 8'hE0. Register-space accesses have no
            // Initial Latency wait (that only applies to memory space), so
            // this doesn't need READ_ST's RWDS-edge synchronization.
            dq_sr <= {8'he0, 8'h00, 8'h01, 8'h00, 8'h00, 8'h00, 16'b0};
            ram_cs_n <= 0;
            ck_e <= 1;
            dq_oen <= 0;
            cycles_sr <= 32'b1;    // start from cycle 0
            state <= CR_READ_ST;
        end
    end

    if (state == CR_READ_ST) begin
        if (cycles_sr[3]) begin
            // 48-bit CA (3 cycles) sent, release the bus for the chip to drive
            dq_oen <= 1;
        end
        if (cycles_sr[4]) begin
            // Register access: no Initial Latency, data is right behind CA.
            cr_read_data  <= dq_in_ris;
            cr_read_valid <= 1'b1;
            ram_cs_n      <= 1;
            ck_e          <= 0;
            state         <= IDLE_ST;
...
// Tristate DDR input.
//
// NOTE: an attempt to insert IODELAY taps here (DQ_DELAY parameter, since
// removed) caused the design to hang immediately (READ_ST never completes -
// it has no timeout and only exits on an RWDS toggle, which never arrived).
// This pin is bidirectional (shared with the ODDR tristate output driver
// above), and the current open-source flow likely doesn't correctly pack
// an IODELAY+IDDR input tap onto the same physical IOB as an existing
// ODDR output on that pin - needs more investigation before retrying.
IDDR iddr_rwds(
